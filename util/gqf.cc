																																										 ...
																																										 uint64_t		 old_length)
{
	...
	while (current_distance > 0) {
		...
		if (current_bucket <= current_slot) {
			...
			//uint64_t i;
			for (uint64_t idx = current_slot; idx < current_slot + current_distance; idx++) {
				set_slot(qf, idx, 0);
				METADATA_WORD(qf, runends, idx) &= ~(1ULL << (idx % 64));
			}
	...
	while (1 && old_length > total_remainders) {	// we only update offsets if we shift/delete anything
		...
		if (last_occupieds_bit == -1) {
			...
			if (runend_index / SLOTS_PER_BLOCK == original_block) { // if the run ends in the same block
				...
				//uint64_t i;
				for (uint64_t idx = original_block + 1; idx < runend_index / SLOTS_PER_BLOCK - 1; idx++)
					get_block(qf, idx)->offset = SLOTS_PER_BLOCK;
