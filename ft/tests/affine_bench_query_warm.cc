...
test_main(int argc, const char *argv[]) {
    ...
    //size_t heuristic = 4*1024;
    //size_t warmup = (4*(4*1024) /nodeMB);
    //size_t warmup_query_rows = warmup < heuristic ? warmup:heuristic ;
    size_t warmup_query_rows = 4*(4*1024/nodeMB);
    random_numbers = warmup_query_rows > query_rows? warmup_query_rows:query_rows ; 
